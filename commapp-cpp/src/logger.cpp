...
std::string getCurrentTime()
{
    ...
    auto hours
      = std::chrono::duration_cast<std::chrono::hours>(since_midnight);
    auto minutes = std::chrono::duration_cast<std::chrono::minutes>(
      since_midnight - hours);
    auto seconds = std::chrono::duration_cast<std::chrono::seconds>(
      since_midnight - hours - minutes);
    auto milliseconds = std::chrono::duration_cast<std::chrono::milliseconds>(
      since_midnight - hours - minutes - seconds);

    std::string hoursStr
      = (hours.count() < 10 ? "0" : "") + std::to_string(hours.count());
    std::string minutesStr
      = (hours.count() < 10 ? "0" : "") + std::to_string(minutes.count());
    std::string secondsStr
      = (hours.count() < 10 ? "0" : "") + std::to_string(seconds.count());
    std::string millisecondsStr = std::to_string(milliseconds.count())
...
#define FORMAT_CONSOLE_BEGIN(color)                                           \
    SetConsoleTextAttribute(winCmdHandler, (int)color);                       \
...
{
#ifdef _WIN32
    SetConsoleTextAttribute(winCmdHandler,
                            (int)CONSOLE_TEXT_COLOR::COLOR_INFO);
#else
    printf("\033[0m");
#endif
}
