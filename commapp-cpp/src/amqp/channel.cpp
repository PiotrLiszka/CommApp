  ...
  _channel(&_connection)
{
    ...
    _channel
      ...
      .onError([](const char* msg)
               { LOG_ERROR("AMQP queue bind failure!", msg); });
}
...
void channel::consume(consumer_callback callback)
{
    ...
    //   .onCancelled([](auto) { LOG_WARNING("Consume cancelled"); })
    //   .onError([](const char* msg) { LOG_ERROR("Consume error! ", msg); });
    _callbacks.push(callback);
}

void channel::declare_consumers()
{
    _channel.onReady(
      [this]()
      {
          LOG_OK("AMQP channel ready");
          ...
          while(!_callbacks.empty())
          {
              ...
              auto onReceived = [this, callback = std::move(callback)](
...
bool channel::opend() const
{
    return _channel.ready() && _channel.usable();
}
} // namespace amqp